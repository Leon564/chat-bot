 ...
 * gigante que antes vivía inline en `ChatService.chat()`. Esta clase NO
 * cambia el comportamiento: el texto de cada bloque es una copia carácter
 * por carácter del original. `buildLegacy()` es una copia literal del
 * template anterior, usada únicamente por el test de caracterización.
 */
@Injectable()
...
SAVE_MEMORY("Información general") ❌`;
  ...
  /**
   * Copia literal del template `systemPrompt` que antes vivía inline en
   * `ChatService.chat()`, con todos los bloques siempre presentes. Existe
   * SOLO para el test de caracterización de la Task 1 (verificar que la
   * extracción a bloques fue fiel) — se borra en la Task 4.
   */
  private buildLegacy(input: PromptInput): string {
    const username = input.username;
    const botName = input.botName;
    const maxLen = input.maxLength;
    const rules = '[scroll] 1. Sé respetuoso [/scroll] [scroll]2. Nada de spam o links sospechosos [/scroll] [scroll] 3. No contenido ilegal 🌀 [/scroll] [scroll] 3. No compartir información personal o redes sociales 🌀 [/scroll] ¡Disfruta del chat y del manga!';

    const useMemory = input.useMemory;
...
- Es ${this.getTimeOfDay(now)} del ${this.getDayType(now)}${specialDayText}`;
    ...
    const generateMemoryExamples = (): string => {
      if (!useMemory) return '';
      return `
...
SAVE_MEMORY("Información general") ❌`;
    ...
    const personaIntro = personality === 'unfiltered'
      ? this.buildUnfilteredPersona(botName, username, maxLen)
      : this.buildDefaultPersona(botName, username, maxLen);
...
CRÍTICO: Incluye SIEMPRE el token {{resumen}} cuando se solicite un resumen, {{usuarios_online}} solo para el roster completo, {{music:<query>}} cuando pidan música, y {{anilist:<tipo>:<título>}} cuando pidan info de un manga/manhwa/manhua/anime concreto.${memoryInstructions}${generateMemoryExamples()}
